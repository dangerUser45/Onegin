...
void Read_File (ONEGIN* file)
{
    ...
    char* buffer_addr = file->buffer_addr = (char*) calloc(fsize + 2 , sizeof(char)); //создал буфер размером двоичного?(да) текста
    assert(buffer_addr);

    fread(buffer_addr, sizeof(char), fsize + 1, fp);                                  //заполнил массив
    buffer_addr[fsize] = '\n';                                                      //записал в последний элемент массива '\0'
    buffer_addr[fsize+1] = EOF;
    ...
    assert(fp);
     ...
     printf("file->fsize = %ld\nbuffer_addr = %p, file->buffer_addr = %p\nbuffer_addr[%ld] = '%c'\n",
            file->fsize, buffer_addr, file->buffer_addr,fsize, buffer_addr[fsize]);
    //
}
