...
void Self_Sort (const void* data, size_t quantity, size_t single_size, Cmpr_fn_t Compare_common,/**/ONEGIN* file);
...
int main (int argc, char* argv[])
{
    ...
    File_Common (&file);
    Self_Sort (file.str_addr, file.string_quantity, sizeof(char*), Strcompare, &file);
    Print_text (&file);
    FREE (&file);
...
void Address_String (ONEGIN* file)
{
    ...
    for (int i = 0; i < n_string; i++)
    {
        ...
        printf("%d) адрес начала строки - str_addr[i] = %llu\n, адрес где хранится это значение = %llu", i+11, str_addr[i], str_addr + i);
        printf("\n");
    }
     */
