...
void File_Common (ONEGIN* file)
{
    ...
    long fsize = file->fsize = file_size (file->name);                   //óçíàë ðàçìåð ôàéëà â ñèìâîëàõ

    Check_fsize (fsize);                                                 //ïðîâåðèë íå ïóñòîé ëè ôàéë
    Read_File (file);                                                    //ïðî÷èòàë ôàéë è ïåðåêîïèðîâàë âñ¸ â áóôåð

    Strings_Number (file);
    Address_String (file);                            //ïîñ÷èòàë êîë-âî ñòðîê
}
...
long file_size (const char* name)
{
    ...
    stat(name, &stbuf);
    //
    printf ("stbuf.st_size = %ld", stbuf.st_size);
    //
...
void Read_File (ONEGIN* file)
{
    ...
    char* buffer_addr = file->buffer_addr = (char*) calloc(fsize + 1 , sizeof(char)); //ñîçäàë áóôåð ðàçìåðîì äâîè÷íîãî?(äà) òåêñòà
    assert(buffer_addr);

    fread(buffer_addr, sizeof(char), fsize + 1, fp);                                  //çàïîëíèë ìàññèâ
